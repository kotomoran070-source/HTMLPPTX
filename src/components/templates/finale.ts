...
export function button(b: FinaleButton, cls: string): string {
  const inner = `<i>${icon(b.icon)}</i><span>${t(b.label)}</span>`;
  return b.url
    ? `<a class="${cls}" href="${esc(b.url)}" target="_blank" rel="noopener" aria-label="${esc(b.label)}">${inner}</a>`
    : `<div class="${cls}" tabindex="0" aria-label="${esc(b.label)}">${inner}</div>`;
}
...
defineTemplate<FinaleSlide>('finale', {
  ...
  render(s, ctx) {
    ...
    const logo = ctx.logo ? `<div class="logo fin-logo"><img src="${esc(ctx.logo)}" alt=""></div>` : '';
    let plate = '';
    if (s.link || s.buttons?.length) {
      const l = s.link;
      const qr = l && l.qr !== false ? `<div class="qr">${qrSvg(l.url, ctx.logo, `QR-код: ${l.url}`)}</div>` : '';
      const primary = l
        ? `<a class="it primary" href="${esc(l.url)}" target="_blank" rel="noopener"><i>${icon('browser')}</i><div>`
          + (l.label ? `<small>${t(l.label)}</small>` : '') + `<b>${t(linkText(l))}</b></div></a>`
        : '';
    ...
    return `<svg class="rg" viewBox="0 0 1280 720" aria-hidden="true">${rings}</svg><div class="beam"></div>`
      + (s.caption ? `<div class="cap">${t(s.caption)}</div>` : '')
      + logo
      + `<h1 aria-label="${esc(s.title)}">${words(s.title, 'w')}</h1>`
      + `<div class="ln2"></div>`
