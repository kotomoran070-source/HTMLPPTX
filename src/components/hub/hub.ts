...
defineBlock<HubProps>('hub', {
  render(p, ctx) {
    ...
    const list = items.map((it, k) =>
      `<div class="hub-item r" data-k="h${k}" tabindex="0"><b>${t(it.title)}</b>${it.text ? t(it.text) : ''}</div>`).join('');
    ...
    items.forEach((it, k) => {
      ...
      nodes += `<g class="nd" data-k="h${k}" tabindex="0"><rect x="${nx.toFixed(1)}" y="${ny.toFixed(1)}" width="${w.toFixed(1)}" height="${h}" rx="12"/>`
        + `<text x="${tx.toFixed(1)}" y="${(ny + (it.sub ? 24 : 25)).toFixed(1)}" text-anchor="middle">${esc(label)}</text>`
        + (it.sub ? `<text class="sb" x="${tx.toFixed(1)}" y="${(ny + 41).toFixed(1)}" text-anchor="middle">${esc(it.sub)}</text>` : '')
        + `</g>`;
    });
    const center = `<rect class="hub-center" x="${CX - 40}" y="${CY - 40}" width="80" height="80" rx="18"/>`
      + (ctx.logo ? `<image href="${esc(ctx.logo)}" x="${CX - 28}" y="${CY - 28}" width="56" height="56"/>` : '');
